                         ...
                         uint32_t       * pBitValue)
{
  ...
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pBitValue != (uint32_t *)NULL)),BSECDRV,AssertBlocking);

  /* Clean up first Error field in handle (used by previous calls to BSEC driver) */
  hBsec->Error = BSEC_ERROR_NONE;

  /* Check validity of input parameters */
  if((regIdx <= BSEC_REG_INDEX_MAX) && (regBit < 32))
  {
    switch(regType)
    {
    ...
    * pBitValue = ((*ptrReg & (1 << regBit)) >> regBit);
  ...
  else
  {
    ...
    hBsec->Error = BSEC_ERROR_INVALID_PARAMETER;
                ...
                BSEC_SafMemClkRangeTypeDef   safMemClkFreqRange)
{
  ...
  if(hBsec->State >= BSEC_STATE_READY)
  {
    ...
    hBsec->Instance->BSEC_OTP_CONFIG |= ((BSEC_SAFMEM_ON << BSEC_OTP_CONFIG_PWRUP_Pos) |
        ((safMemClkFreqRange & BSEC_OTP_CONFIG_FRC) << BSEC_OTP_CONFIG_FRC_Pos));
    ...
    while(((hBsec->Instance->BSEC_OTP_STATUS & BSEC_OTP_STATUS_PWRON) == 0) &&
        (timeoutDetected == 0))
    {
      /* Sense timeout occurrence */
      if((HAL_GetTick() - timerValInit) >= BSEC_TIMEOUT_SAFMEM_PWRUP_VAL)
      {
        timeoutDetected = 1;
      }
    ...
    else
    {
      ...
      hBsec->Error |= BSEC_ERROR_TIMEOUT;
    }
  ...
  else
  {
    ...
    /* Update Error description */
    hBsec->Error |= BSEC_ERROR_DRIVER_NOT_READY;
  }
                  ...
                  uint32_t       * pOtpShadowVal)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pOtpShadowVal != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
              ...
              uint32_t       * pOtpShadowVal)
{
  ...
  if(hBsec->State == BSEC_STATE_SAFMEM_PWRUP)
  {
    ...
    {
      /* Update Error description */
      hBsec->Error |= BSEC_ERROR_SOFT_DRIVER;
    ...
    if(bitValueDisturbed == 1)
    {
      /* Update Error description */
      hBsec->Error |= BSEC_ERROR_DISTURBED;
    ...
    if(bitValueError == 1)
    {
      ...
      hBsec->Error |= BSEC_ERROR_ECC_REDUNDANCY;
  ...
  else
  {
    ...
    /* Update Error description */
    hBsec->Error |= BSEC_ERROR_SAFMEM_PWRDOWN;
...
HAL_StatusTypeDef HAL_BSEC_GetSecurityStatus(BSEC_HandleTypeDef * hBsec, BSEC_ChipSecurityTypeDef * pSecStatus)
{
  ...
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pSecStatus != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
  ...
  if(hBsec->State >= BSEC_STATE_READY)
  {
    ...
    if((otpStatus & BSEC_OTP_STATUS_INVALID) != 0 || (otpStatus & BSEC_OTP_STATUS_FULLDBG) != 0)
    {
    ...
    else if ((otpStatus & BSEC_OTP_STATUS_CLOSED) != 0)
    {
    ...
    if(hBsec->State >= BSEC_STATE_READY)
    {
      ...
      if(status == HAL_OK)
      {
        ...
        else
        {
          ...
          /* Update Error description */
          hBsec->Error |= BSEC_ERROR_LOCK_WRITE;
        }
...
*      1b- Check if OTP word is not sticky programming locked, in that case return an error BSEC_ERROR_STICKY_LOCK_PROG.
...
*      3a- If step 2b exits without error, check if 'otpWordValReq' is a value possible to write given SAFMEM technology,
                 ...
                 uint32_t         otpWordValReq)
{
  ...
  uint32_t permProgLockStatus = BSEC_NOTLOCKED;
  uint32_t stickyLockStatus = BSEC_NOTLOCKED;
  uint32_t stickyReadLockBitValue = 0;
  __IO uint32_t otpShadowValBeforeProg = 0;
  __IO uint32_t otpShadowValAfterProg = 0;
  ...
  if(hBsec->State == BSEC_STATE_SAFMEM_PWRUP)
  {
    ...
    if(permProgLockStatus == BSEC_LOCKED)
    {
      /* Update Error description : permanent programming lock of that OTP word */
      hBsec->Error |= BSEC_ERROR_PERM_LOCK_PROG;
    ...
    if((stickyLockStatus & BSEC_OTP_STICKY_LOCK_PROG_FUSE) != 0)
    {
      /* Update Error description : sticky programming lock of that otp word */
      hBsec->Error |= BSEC_ERROR_STICKY_LOCK_PROG;
    ...
    if(stickyReadLockBitValue == 0)
    {
      ...
      for(bit = 0; ((bit < 32) && (impossibleValDetected == 0)); bit++)
      {
        /* Look for bits different in 'otpShadowValBeforeProg' and 'otpWordValReq' */
        if((otpDiffWord & (1 << bit)) != 0)
        {
          /* bit is different */
          if(((otpShadowValBeforeProg & (1 << bit)) != 0)  &&
            ((otpWordValReq & (1 << bit)) == 0))
          {
            /* This is a 1 -> 0 transition : impossible with SAFMEM technology ! */
            impossibleValDetected = 1;
          }
      ...
      if(impossibleValDetected == 1)
      {
        ...
        hBsec->Error |= BSEC_ERROR_PROG_VALUE_NOT_POSSIBLE;
    ...
    if((hBsec->Instance->BSEC_OTP_STATUS & BSEC_OTP_STATUS_PROGFAIL) != 0)
    {
      /* PROGFAIL indication : programming was not successful */
      /* Update Error description */
      hBsec->Error |= BSEC_ERROR_PROG_FAIL;
...
HAL_StatusTypeDef HAL_BSEC_ReadScratch(BSEC_HandleTypeDef * hBsec, uint32_t * pScratchReadVal)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pScratchReadVal != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
...
HAL_StatusTypeDef HAL_BSEC_JtagInRead(BSEC_HandleTypeDef * hBsec, uint16_t * pJtagReadVal)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pJtagReadVal != (uint16_t *)NULL)),BSECDRV,AssertBlocking);
...
{
  HAL_StatusTypeDef retVal = HAL_ERROR;
  uint32_t maxFuncLockFieldVal = (BSEC_FUNC_STICKY_LOCK_PROG_GLOBAL_OTP |
                  BSEC_FUNC_STICKY_LOCK_ROMLOCK     |
                  BSEC_FUNC_STICKY_LOCK_DENABLE_WRITE   |
                  BSEC_FUNC_STICKY_LOCK_OTP_UPPER_READ);
  uint32_t setFunctionMaskEffective = 0;
...
HAL_StatusTypeDef HAL_BSEC_FunctionGetLock(BSEC_HandleTypeDef * hBsec, uint32_t * pFunctionLockMaskRead)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pFunctionLockMaskRead != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
      ...
      if(status == HAL_OK)
      {
        if(lockStatus == BSEC_NOTLOCKED)
        {
          ...
          else
          {
            ...
            HAL_BSEC_Div((otpWordIdx - 32), 16, &divVal, &modVal);
            otpAddr = (divVal + 0x04);
            otpWrData = (0x1 << modVal);
          }
        ...
        {
          /* Update Error description */
          hBsec->Error |= BSEC_ERROR_ALREADY_LOCKED;
        }
                          ...
                          uint32_t       * pLockStatus)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pLockStatus != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
    ...
    if(hBsec->State >= BSEC_STATE_READY)
    {
      ...
      if(status == HAL_OK)
      {
        if(bitValue == 0)
        {
          ...
          * pLockStatus = BSEC_NOTLOCKED;
        }
        else
        {
          /* OTP word 'otpWordIdx' is permanently write locked */
          * pLockStatus = BSEC_LOCKED;
        }
...
HAL_StatusTypeDef HAL_BSEC_SetOtpStickyLock(BSEC_HandleTypeDef * hBsec, uint32_t otpWordIdx, uint32_t stickyLockCmd)
{
  ...
  if((otpWordIdx <= BSEC_OTP_WORD_INDEX_MAX) &&
     (stickyLockCmd <= (BSEC_OTP_STICKY_LOCK_PROG_FUSE |
              BSEC_OTP_STICKY_LOCK_WRITE_SHADOW  |
              BSEC_OTP_STICKY_LOCK_UPDATE_SHADOW)))
  {
    /* The BSEC driver should be at least in ready state */
    if(hBsec->State >= BSEC_STATE_READY)
    {
      ...
      if((stickyLockCmd & BSEC_OTP_STICKY_LOCK_PROG_FUSE) != 0)
      {
      ...
      if((stickyLockCmd & BSEC_OTP_STICKY_LOCK_UPDATE_SHADOW) != 0)
      {
                       ...
                       uint32_t       * pStickyLockStatus)
{
  ...
  __IO uint32_t * ptrReg1 = (uint32_t *)NULL;
  __IO uint32_t * ptrReg2 = (uint32_t *)NULL;
  __IO uint32_t * ptrReg3 = (uint32_t *)NULL;

  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pStickyLockStatus != (uint32_t*)NULL)),BSECDRV,AssertBlocking);
  ...
  if(otpWordIdx <= BSEC_OTP_WORD_INDEX_MAX)
  {
    ...
    if(hBsec->State >= BSEC_STATE_READY)
    {
      ...
      splock_flag = (* ptrReg1 & (1 << regBit));
      swlock_flag = (* ptrReg2 & (1 << regBit));
      srlock_flag = (* ptrReg3 & (1 << regBit));

      /* Check sticky lock programming status */
      if(splock_flag != 0)
      {
        stickyLockStatus |= BSEC_OTP_STICKY_LOCK_PROG_FUSE;
      }
      ...
      if(srlock_flag != 0)
      {
        stickyLockStatus |= BSEC_OTP_STICKY_LOCK_UPDATE_SHADOW;
      }
...
{
  HAL_StatusTypeDef status = HAL_ERROR;
  uint32_t featureDisabledValidMask = ((1 << (BSEC_FEAT_MAX + 1)) - 1);
  __IO uint32_t otpShadowReadVal = 0;

  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pMaskFeatDisabled != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
...
HAL_StatusTypeDef HAL_BSEC_GetFdisTkValue(BSEC_HandleTypeDef * hBsec, uint32_t * pFdisTkValue)
{
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pFdisTkValue != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
  ...
  if(status == HAL_OK)
  {
    * pFdisTkValue = ((otpShadowReadVal & (1 << BSEC_OTP_CFG1_FDIS_TK_BIT_POS)) >> BSEC_OTP_CFG1_FDIS_TK_BIT_POS);
  }
...
HAL_StatusTypeDef HAL_BSEC_GetTk(BSEC_HandleTypeDef * hBsec, uint32_t * tkValue)
{
  ...
  uint32_t * pReadPointer = &(tkValue[3]); /* Initialize read pointer to key LSWord start position for the key retrieve */
  ...
  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (tkValue != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
...
{
  HAL_StatusTypeDef retVal = HAL_ERROR;
  uint32_t validMask = ((1 << (BSEC_DEBUG_ENABLE_MAX + 1)) - 1);

  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pDebugMaskEnabled != (uint32_t *)NULL)),BSECDRV,AssertBlocking);
...
HAL_StatusTypeDef HAL_BSEC_SetDebugEnabled(BSEC_HandleTypeDef * hBsec, uint32_t debugMaskEnabled)
{
  ...
  if(debugMaskEnabled <= validMask)
  {
    ...
    if(hBsec->State >= BSEC_STATE_READY)
    {
      ...
      if((hBsec->Instance->BSEC_OTP_LOCK & BSEC_OTP_LOCK_DENREG) != 0)
      {
               ...
               const uint8_t    * pPKH)
{
  HAL_StatusTypeDef retVal = HAL_ERROR;
  HAL_StatusTypeDef status = HAL_OK;
  __IO uint32_t * pPubKeyHashWord = (__IO uint32_t *)NULL;
  uint32_t otpWordIdx = 0;

  /* Verify input parameters validity */
  //INFASSERTS_REQUIRE(((hBsec != (BSEC_HandleTypeDef *)NULL) && (pPKH != (uint8_t *)NULL)),BSECDRV,AssertBlocking);
  ...
  if(pkhOtpFirstWordIdx <= BSEC_OTP_WORD_INDEX_MAX)
  {
    ...
    if(readMode == BSEC_READ_FROM_SHADOW)
    {
      ...
      if(hBsec->State >= BSEC_STATE_READY)
      {
        for(otpWordIdx = pkhOtpFirstWordIdx; ((otpWordIdx < (pkhOtpFirstWordIdx + 8)) && (status == HAL_OK)); otpWordIdx++)
        {
...
******************************************************************************
*/
uint32_t HAL_BSEC_WaitBusyClearWithTimeout(BSEC_HandleTypeDef * hBsec)
{
  uint32_t timerValInit = 0;
  uint32_t timeoutDetected = 0;
  ...
  while(((hBsec->Instance->BSEC_OTP_STATUS & BSEC_OTP_STATUS_BUSY) != 0) &&
       (timeoutDetected == 0))
  {
    /* Sense timeout occurrence */
    if((HAL_GetTick() - timerValInit) >= BSEC_TIMEOUT_VAL)
    {
      timeoutDetected = 1;
    }
