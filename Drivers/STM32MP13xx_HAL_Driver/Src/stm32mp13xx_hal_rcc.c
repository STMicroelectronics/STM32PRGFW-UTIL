...
HAL_StatusTypeDef RCC_MLAHBConfig(RCC_MLAHBInitTypeDef *MLAHBInitStruct);
/**
  * @}
  */
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(RCC_ClkInitTypeDef  *RCC_ClkInitStruct)
{
  ...
  if ((RCC_ClkInitStruct->ClockType & RCC_CLOCKTYPE_PCLK6) == RCC_CLOCKTYPE_PCLK6)
  {
    ...
    while (__HAL_RCC_GET_FLAG(RCC_FLAG_APB6DIVRDY) == RESET)
    {
      if ((HAL_GetTick() - tickstart) > CLOCKSWITCH_TIMEOUT_VALUE)
      {
        return HAL_TIMEOUT;
...
HAL_StatusTypeDef RCC_MLAHBConfig(RCC_MLAHBInitTypeDef *MLAHBInitStruct)
{
  ...
  while (__HAL_RCC_GET_FLAG(RCC_FLAG_MLAHBSSRCRDY) == RESET)
  {
    if ((HAL_GetTick() - tickstart) > CLOCKSWITCH_TIMEOUT_VALUE)
    {
      return HAL_TIMEOUT;
  ...
  /* Wait till MLAHB is ready */
  while (__HAL_RCC_GET_FLAG(RCC_FLAG_MLAHBDIVRDY) == RESET)
  {
    if ((HAL_GetTick() - tickstart) > CLOCKSWITCH_TIMEOUT_VALUE)
    {
      return HAL_TIMEOUT;
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef  *RCC_ClkInitStruct, uint32_t *pFLatency)
{
  ...
  RCC_ClkInitStruct->ClockType = RCC_CLOCKTYPE_MPU | RCC_CLOCKTYPE_ACLK | RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_PCLK4 |
                                 RCC_CLOCKTYPE_PCLK5 | RCC_CLOCKTYPE_PCLK1 | RCC_CLOCKTYPE_PCLK2 | RCC_CLOCKTYPE_PCLK3 | RCC_CLOCKTYPE_PCLK6;
