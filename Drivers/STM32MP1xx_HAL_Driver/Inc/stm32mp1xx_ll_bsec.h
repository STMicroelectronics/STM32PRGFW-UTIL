...
#ifndef STM32MP1xx_LL_BSEC_H
#define STM32MP1xx_LL_BSEC_H
...
{
    return ( ((BSEC_READ(BSEC_OTP_STATUS) & BSEC_OTP_STATUS_INVALID) == 0) ? 1 : 0);
}
...
{
  BSEC->BSEC_OTP_CONFIG |= (1 << BSEC_OTP_CONFIG_PWRUP_Pos);
  while( (BSEC->BSEC_OTP_STATUS & BSEC_OTP_STATUS_PWRON) == 0);
}
...
__STATIC_INLINE void LL_BSEC_SetSecureDebugEnableBits( uint32_t mask)
{
    ...
    mask &= (BSEC_DENABLE_SPIDEN | BSEC_DENABLE_SPNIDEN);
