...
#ifndef STM32MP1xx_HAL_BSEC_H
#define STM32MP1xx_HAL_BSEC_H
 ...
 * The current duration of timeout 1.56 ms can be too short to allow a special word programming with many bits set to 1'.
 *
 * conclusion :
...
typedef enum
{
  /*!< Feature CRYP : cryptographic Hw accelerator that is CRYP for export control                         */
  /* No impact on other cryptographic relative operations such as HASH & RNG                               */
  /* When 'Crypto_disable' BSEC_OTP_DATA1 bit is set the 3DES & AES IPs are not clocked and cannot be used */
  /* to accelerate by Hw a decryption 3DES or AES */
  BSEC_FEAT_CRYP    = BSEC_FENABLE_CRYP_FIELD_SHIFT,

  /*!< Feature CAN usable ?  */
  BSEC_FEAT_CAN     = BSEC_FENABLE_CAN_FIELD_SHIFT,

  /*!< Feature DSI usable ?  */
  BSEC_FEAT_DSI     = BSEC_FENABLE_DSI_FIELD_SHIFT,

  BSEC_FEAT_MAX     = BSEC_FEAT_DSI

} BSEC_OtpFeatureStickyLockTypeDef;
#endif
/**
  ...
  * DEVICEEN : CoreSight signal : allow/disallow access to DAP APB-AP by external tool connection (hence APB part of debug).
  ...
  * CP15SDISABLE0 : signal influenced by BSEC programming and going to CA7 MPCore cluster core 0 to allow/disallow programming of
  *                 CP15 secure registers on core 0.
  * CP15SDISABLE1 : signal influenced by BSEC programming and going to CA7 MPCore cluster core 1 to allow/disallow programming of
  *                 CP15 secure registers on core 1.
  * CFGSDISABLE : signal influenced by BSEC programming and going to CA7 MPCore cluster to allow/disallow access to GIC Secure registers.
...
HAL_StatusTypeDef HAL_BSEC_UtilReadRegisterBit(BSEC_HandleTypeDef * hBsec, BSEC_RegTypeTypeDef regType, uint32_t regIdx, uint32_t regBit, uint32_t * pBitValue);
HAL_StatusTypeDef HAL_BSEC_ReadRegisterBit(BSEC_HandleTypeDef * hBsec, BSEC_RegTypeTypeDef regType, uint32_t otpWordIdx, uint32_t * pBitValue);
...
HAL_StatusTypeDef HAL_BSEC_GetOtpDisturbStatus(BSEC_HandleTypeDef * hBsec, uint32_t otpWordIdx, uint32_t * pDisturbStatus);
...
/* Do not use feature re-enabling in Wildcat                                              */
/* Feature re-enabling is sticky write locked by bootROM to prevent its Hw use in product */
/* No existing customer flow in place to re-enable dynamically features                   */
#if 0
HAL_StatusTypeDef HAL_BSEC_FeatureRenable(BSEC_HandleTypeDef * hBsec, uint32_t featureMaskReenable);
#endif /* if 0 */
